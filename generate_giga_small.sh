...
CUDA_VISIBLE_DEVICES=1 python3 generate.py data_giga_bin_small --path checkpoints_small/topic_test/checkpoint_best.pt -enable_topic --beam 5 --batch-size 128 --remove-bpe | tee > ./output/result
###CUDA_VISIBLE_DEVICES=1 python3 generate.py data_giga_bin_small --path checkpoints_small/topic_add/checkpoint_best.pt --beam 5 --batch-size 128 --remove-bpe | tee > ./output/result
